...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemyHP <= 0) //verifica se o Hp do inimigo estiver igual ou abaixo de 0
        {
            ...
            transform.position = new Vector3(0.15f, 2, -2.01f); //coloca o inimigo na posição que o item vai spawnar, já q o spawn do item usa de parametro a posição do inimigo
            Instantiate(item, transform.position, transform.rotation); //spawna o item
            enemyHP = 3; //Reseta o Hp do inimigo para esse if não ficar rodando infinitamente
            transform.position = new Vector3(2.68f, 2, -2); //Reseta a posição do inimigo pra respawnar ele no lugar certo
